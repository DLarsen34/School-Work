...
IF EXISTS (SELECT * FROM sys.tables WHERE name = N'PbRentalHistory')
	DROP TABLE PbRentalHistory;

IF EXISTS (SELECT * FROM sys.tables WHERE name = N'PbReservationHistory')
	DROP TABLE PbReservationHistory;
...
CREATE TABLE PbRentalHistory (
	pbRentalHistory_id int NOT NULL IDENTITY(1,1),
	pbUser_id int NOT NULL,
	pbMovieItem_id int NOT NULL,
	rentalDate dateTime2 NOT NULL,
	returnDate dateTime2,
...
CREATE TABLE PbReservationHistory (
	pbMovie_id int NOT NULL,
	pbUser_id int NOT NULL,
	reservationDate dateTime2 NOT NULL,
	movieType nvarchar(1) NOT NULL,
...
ALTER TABLE PbRentalHistory
	ADD CONSTRAINT PK_PbRentalHistory_PbRentalHistory_id PRIMARY KEY CLUSTERED (pbRentalHistory_id);

ALTER TABLE PbReservationHistory
	ADD CONSTRAINT PK_PbReservationHistory_PbMovie_id_PbUser_id_reservationDate_movieType PRIMARY KEY CLUSTERED (pbMovie_id, pbUser_id, reservationDate, movieType);
...
ALTER TABLE PbReservationHistory
	ADD CONSTRAINT FK_PbReservationHistory_PbMovie_id_Pb_user_id_reservationDate_movieType FOREIGN KEY (pbMovie_id, pbUser_id, reservationDate, movieType) REFERENCES PbMovieReservation (pbMovie_id, pbUser_id, reservationDate, movieType);
...
ALTER TABLE PbRentalHistory
	ADD CONSTRAINT AK_PbRentalHistory_pbUser_id_pbMovieItem_id_rentalDate UNIQUE (pbUser_id, pbMovieItem_id, rentalDate);
...
IF EXISTS (
	...
	WHERE SPECIFIC_NAME = 'usp_adUser'
	)
...
EXECUTE usp_addUser @userID = 'PB0000678', @userFirstName = 'Logan', @userLastName = 'Larsen', @userPassword = 'RunMeDown', @userPhoneNumber = '8016586765', @userPhoneNumber2 = NULL, @userType = 'C', @customerType = 'S', @banStatus = 'B', @fees = 0.00;
------------------------------------
--PbQuestion
------------------------------------
IF EXISTS (
...
EXECUTE usp_addUserQuestion @userID = 'PB0000027', @Question = 'What is the name of the first pet you owned?', @Answer = 'Rorke'; 
...
IF EXISTS (
	SELECT * FROM INFORMATION_SCHEMA.ROUTINES
	WHERE SPECIFIC_NAME = 'usp_MovieReservation'
	)
	DROP PROCEDURE usp_MovieReservation;
	GO
CREATE PROCEDURE dbo.usp_MovieReservation
@userID nvarchar(9),
@movieID nvarchar(20),
@movieType nvarchar(1)
AS
...
DECLARE @pbMovie_id int;
	...
	FROM PbMovie
	WHERE movieID = @movieID
IF(@userID IS NULL) PRINT ('Invalid UserID')
IF(@movieID IS NULL) PRINT ('Invalid MovieID')
IF(@movieType IS NULL) PRINT ('Invalid MovieType')
IF(@pbUser_id IS NULL)
		PRINT('Invalid KeywordID');
	IF(@pbMovie_id IS NULL)
		PRINT('Invalid MovieID');
BEGIN TRY
IF (SELECT banStatus FROM PbUser WHERE PbUser_id = (dbo.udf_getUserID(@userID))) = 'N'
IF (SELECT customerType FROM PbUser WHERE pbUser_id =(dbo.udf_getUserID(@userID))) = 'P'
IF (SELECT COUNT(*) FROM PbMovieReservation WHERE PbUser_id = (dbo.udf_getUserID(@userID))) < 4
INSERT INTO PbMovieReservation
(pbMovie_id, pbUser_id, reservationDate, reservationStatus, movieType, lastUpdatedBy, lastUpdatedDate)
VALUES
(@pbMovie_id, @pbUser_id, GETDATE(), 'P', @movieType, 'DLarsen', GETDATE());
END TRY
BEGIN CATCH
...
IF EXISTS (
	SELECT * FROM INFORMATION_SCHEMA.ROUTINES
	WHERE SPECIFIC_NAME = 'usp_MovieRental'
	)
	DROP PROCEDURE usp_MovieRental;
	GO
CREATE PROCEDURE dbo.usp_MovieRental
@userID nvarchar(9),
@movieItemID int
AS
BEGIN
...
DECLARE @pbMovieItem_id int;
	...
	FROM PbMovieItem 
	WHERE pbMovieItem_id = @movieItemID
IF(@pbUser_id IS NULL) PRINT ('Invalid pbUserID')
IF(@pbUser_id IS NULL) PRINT ('Invalid pbmovieItemID')
IF(@userID IS NULL) PRINT ('Invalid UserID')
BEGIN TRY
IF (SELECT banStatus FROM PbUser WHERE PbUser_id = (dbo.udf_getUserID(@userID))) = 'N'
IF (SELECT COUNT(*) FROM PbMovieRental WHERE PbUser_id = (dbo.udf_getUserID(@userID))) < 4
INSERT INTO PbMovieRental
(pbUser_id, pbMovieItem_id, rentalDate, returnDate, dueDate, lastUpdatedBy, lastUpdatedDate)
VALUES
(@pbUser_id, @pbMovieItem_id, GETDATE(), NULL, GETDATE()+3, 'DLarsen', GETDATE())
END TRY
BEGIN CATCH
...
IF EXISTS (
	...
	GO
CREATE FUNCTION dbo.udf_getUserID(@userID nvarchar(9))
RETURNS int
AS 
BEGIN
	DECLARE @pbUser_id int;
...
IF EXISTS (
	...
	GO
CREATE FUNCTION dbo.udf_getMovieItemID(@movieID nvarchar(20), @movieType nvarchar(1), @copyNumber int)
RETURNS int
AS
BEGIN
	DECLARE @pbMovieItem_id int;
	SELECT @pbMovieItem_id = pbMovieItem_id
	FROM PbMovieItem 
	WHERE pbMovie_id =(SELECT pbMovie_id FROM PbMovie WHERE movieID = @movieID)
...
IF EXISTS (
	...
	GO
CREATE FUNCTION dbo.udf_getMovieID(@Title nvarchar(255))
RETURNS int
AS
BEGIN
	DECLARE @pbMovie_id int
...
--FUNCTION CALLS
------------------------------
--SELECT dbo.udf_getUserID('PB0000027') AS 'pb_userID';
--SELECT dbo.udf_getMovieItemID('SonKElder', 'B', 1) AS 'pbMovieItem_id';
--SELECT dbo.udf_getMovieID('True Grit') AS 'pbMovie_id';
--SELECT dbo.udf_getActorID('Chris', 'Evans') AS 'pbActor_id';
--SELECT dbo.udf_getDirectorID('George', 'Lucas') AS 'pbDirector_id';
--SELECT dbo.udf_getGenreID('Musical') AS 'pbGenre_id';
...
--WHERE id = OBJECT_ID(N'udv_actorMovie'))
--DROP VIEW udv_actorMovie;
--GO
--CREATE VIEW dbo.udv_actorMovie
--AS
--SELECT a.actorFirstName AS "Actor First Name", a.actorLastName AS "Actor Last Name", m.Title AS "Movie Title"
--FROM PbActor a INNER JOIN PbMovieActor ma
--ON a.pbActor_id = ma.pbActor_id
--INNER JOIN PbMovie m
--ON m.pbMovie_id = ma.pbMovie_id;
--GO
--SELECT * FROM dbo.udv_actorMovie;

IF EXISTS
(SELECT * FROM sysobjects
WHERE id = OBJECT_ID(N'udv_rentalHistory'))
DROP VIEW dbo.udvrentalHistory;
GO
CREATE VIEW dbo.udv_rentalHistory
AS
SELECT d.pbMovieItem_ID AS "Movie Item ID", b.Title AS "Title", c.pbUser_id AS "PbUserID", c.userFirstName, c.UserLastName, a.rentalDate, a.dueDate
FROM PbUser c INNER JOIN PbMovieRental a
ON c.pbUser_id = a.pbUser_id
INNER JOIN PbMovieItem d
ON d.pbMovieItem_id = a.pbMovieItem_id
INNER JOIN pbMovie b
ON d.pbMovie_id = b.pbMovie_id;
GO
--------------------------------
--Triggers
--------------------------------
...
--AS 
--BEGIN
--INSERT INTO dbo.PbMovieReservationHistory
--	(pbUser_id, pbMovie_id, movieType, reservationDate, reservationStatus, lastUpdatedBy, lastUpdatedDate)
--	(SELECT pbUser_id, pbMovie_id, movieType, reservationDate, reservationStatus, lastUpdatedBy, lastUpdatedDate
--	FROM inserted)
--END
...
--Valid
---------------------
EXECUTE usp_MovieReservation @movieID = 'TRGRT', @userID = 'PB0000001', @movieType = 'B';
EXECUTE usp_MovieReservation @movieID = 'StrWrsIVNwHp', @userID = 'PB0000001', @movieType = 'B';
EXECUTE usp_MovieReservation @movieID = 'SonKElder', @userID = 'PB0000001', @movieType = 'B';
EXECUTE usp_MovieReservation @movieID = 'XMEN', @userID = 'PB0000001', @movieType = 'B';


-------------------
--invalid
------------------
EXECUTE usp_MovieReservation @movieID = 'Deadpl', @userID = 'PB0000001', @movieType = 'B';
EXECUTE usp_MovieReservation @movieID = 'Deadpl', @userID = 'PB0000678', @movieType = 'D';


------------------------
--Valid
------------------------
EXECUTE usp_MovieRental @userID = 'PB0000001', @movieItemid = 1;
EXECUTE usp_MovieRental @userID = 'PB0000001', @movieItemid = 5;
EXECUTE usp_MovieRental @userID = 'PB0000001', @movieItemid = 3;
EXECUTE usp_MovieRental @userID = 'PB0000001', @movieItemid = 7;

------------------------
--Invalid
------------------------
EXECUTE usp_MovieRental @userID = 'PB0000001', @movieItemid = 9;
EXECUTE usp_MovieRental @userID = 'PB0000678', @movieItemid = 8;

-----------------
--Rentals
----------------
EXECUTE usp_MovieRental @userID = 'PB0000021', @movieItemid = 23;
EXECUTE usp_MovieRental @userID = 'PB0000021', @movieItemid = 12;
EXECUTE usp_MovieRental @userID = 'PB0000027', @movieItemid = 21;
EXECUTE usp_MovieRental @userID = 'PB0000101', @movieItemid = 13;
EXECUTE usp_MovieRental @userID = 'PB0000232', @movieItemid = 27;
EXECUTE usp_MovieRental @userID = 'PB0000232', @movieItemid = 18;
