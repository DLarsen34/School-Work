//Copyright 2018, Bradley Peterson, Weber State University, all rights reserved. (10/11/2018)
...
arrayHashTable<KEY, VALUE>::arrayHashTable(arrayHashTable<KEY, VALUE>&& obj) {
	...
	arrayHashTable<KEY, VALUE> temp;
	temp.keyArray = std::move(obj.keyArray);
	temp.valueArray = std::move(obj.valueArray);
	temp.statusArray = std::move(obj.statusArray);
	temp.arraySize = obj.arraySize;
	obj.arraySize = 0;
	return temp;
}
...
arrayHashTable<KEY, VALUE>& arrayHashTable<KEY, VALUE>::operator=(arrayHashTable<KEY, VALUE>&& obj) {
	//TODO: MOVE all data member objects.  
	//For primitives (like ints), copy these date members and then set the original to a default value (like 0)
...
void arrayHashTable<KEY, VALUE>::add(const KEY& key, const VALUE& value) {
	...
	std::hash<KEY> hashFunctor;
	int index = hashFunctor(key) % arraySize;

	if ((statusArray[index] == -1) || (statusArray[index] == 0))
	{
		keyArray[index] = key;
...
void arrayHashTable<KEY, VALUE>::add(const KEY& key, VALUE&& value) {
	...
	{
		keyArray[index] = key;
		valueArray[index] = std::move(value);
		statusArray[index] = 1;
	}
	else
	{
...
bool arrayHashTable<KEY, VALUE>::exists(const KEY& key) const {
	...
	if ((statusArray[index] == 1) && (keyArray[index] == key))
		return true;
	else
	{
		for (int i = (index + 1); i <= arraySize; i++)
		{
			if ((statusArray[i] == 1) && (keyArray[i] == key))
			{
				return true;
				break;
			}
...
VALUE& arrayHashTable<KEY, VALUE>::operator[](const KEY& key) {
	...
	else if ((statusArray[index] == 0) || (statusArray[index] == -1))
		throw Error();
...
void testSimpleIntHash() {
	...
	//Test #1, add "Nuggets" into our hash with a key of 6.  Try to retrive it.
	myHash.add(24006, "Nuggets");
	checkTest("testSimpleIntHash #1", "Nuggets", myHash.item(24006));

	//Test #2, attempt to get the Jazz using the operator[] code.
	checkTest("testSimpleIntHash #2", "Nuggets", myHash[24006]);

	//Test #3, attempt to change the value at this location.  It should overwrite.
	myHash[24006] = "Jazz";
	checkTest("testSimpleIntHash #3", "Jazz", myHash[24006]);
	...
	// Now attempt to iterate over what's left.  This should show 9 items
	cout << "This iteration loop should display 9 items" << endl;
	count = 0;
	for (auto& team : myHash) {
		cout << team << " ";
		count++;
	}
	checkTest("testSimpleIntHash #22", 9, count);
}
